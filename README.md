...
TaBiAI（注册链接域名为 `tabitoken.com`）公开展示 Chat、Responses、Claude 和 Gemini 等协议入口，并提供 Cherry Studio、CC Switch 的适配入口。核验时模型广场显示 4 个当前启用的 Claude 模型，按请求计费，并展示延迟、吞吐和成功率等站内指标。公开注册页暂未展示固定注册送额度。

> **立即注册：**[TaBiAI 邀请注册链接](https://tabitoken.com/sign-up?aff=QF6b)

### 9. 熊猫API：多个端点、模型监控和公开教程

熊猫 API 公开给出 `https://api520.pro` 主站端点，作为 OpenAI 兼容端点时末尾加 `/v1`，并提供备用入口、模型监控、令牌额度查询和接入教程。当前公开福利说明为新用户 100 次试用额度，邀请双方各得 100 次额度；模型广场同时展示 Claude、DeepSeek、Gemini、智谱等分类。活动和额度以后台实际到账为准。

> **立即注册：**[熊猫API 邀请注册链接](https://api520.pro/register?aff=I45H)

### 10. 橘子API：明确端点、按量计费和可用性检测

橘子 API 公开接口地址为 `https://api.juziai.cc/v1`，首页说明支持 Claude、GPT、DeepSeek 等主流模型，采用按量计费并提供模型可用性检测。公开页面未展示固定注册送额度，具体模型和价格以控制台为准。
